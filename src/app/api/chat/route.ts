...
import { buildSystemPrompt, buildRetrievalSystemAppendix } from "@/lib/system-prompt";
import { getVideoTranscript } from "@/lib/files";
import { formatTimestamp } from "@/lib/utils";
import { retrieveManualChunks, formatRetrievalForPrompt } from "@/lib/manual-retrieval";
import { runManagerReview, extractTextFromMessage } from "@/lib/accessor-manager";
...
function videoContextBlock(): string {
  ...
  const lines = chunks.slice(0, 50).map(
    (c) => `[${formatTimestamp(c.startTime)}-${formatTimestamp(c.endTime)}] ${c.text}`
  );
  return lines.join("\n").slice(0, 8000);
}
    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          controller.enqueue(
            encoder.encode(
              sseEvent("phase", { id: "retrieving", label: "Searching manual…" })
            )
          );

          const hits = await retrieveManualChunks(queryForRetrieval, { limit: 10 });
          const retrievalBlock = formatRetrievalForPrompt(hits);
          const videoSummary = videoContextBlock();

          const system =
            buildSystemPrompt("text") +
            buildRetrievalSystemAppendix(retrievalBlock, videoSummary || undefined);

          controller.enqueue(
            encoder.encode(sseEvent("phase", { id: "thinking", label: "Reasoning…" }))
          );
          ...
          while (iterations < MAX_TOOL_ITERATIONS) {
            ...
            for await (const event of messageStream) {
              ...
              if (event.type === "content_block_delta") {
                if (event.delta.type === "text_delta") {
                  controller.enqueue(
                    encoder.encode(sseEvent("text", { text: event.delta.text }))
                  );
                }
                const delta = event.delta as unknown as { type: string; citation?: unknown };
                if (delta.type === "citations_delta" && delta.citation) {
                  controller.enqueue(
                    encoder.encode(sseEvent("citation", { citation: delta.citation }))
                  );
                }
              }
            }
            ...
            for (const tc of toolCalls) {
              ...
              controller.enqueue(encoder.encode(sseEvent("tool_result", { name: tc.name })));
            }
          ...
          if (!managerDisabled && draft.trim().length > 0) {
            controller.enqueue(
              encoder.encode(sseEvent("phase", { id: "reviewing", label: "Validating answer…" }))
            );
            ...
            if (verdict.verdict === "revise" && verdict.issues.length > 0) {
              controller.enqueue(
                encoder.encode(
                  sseEvent("phase", { id: "revising", label: "Applying review feedback…" })
                )
              );
          ...
          controller.enqueue(encoder.encode(sseEvent("phase", { id: "idle", label: "" })));
